...
### 5.1 后端框架架构优化说明
后端采用**按业务模块（Feature-based）划分**的架构设计，这比传统的按技术层划分（Controller/Service/Mapper分开）具有更好的模块内聚性，便于后续微服务拆分或代码维护。吸收了老师模板中“中间件、统一响应、错误码、初始化脚本”的思想。

1. **按业务高内聚**：将同一业务（如 `decision`）下的实体类、Controller、Service、Mapper、DTO 聚拢在一个包内，避免修改一个功能需要在多个根目录下跳转。
2. **Controller 层只负责接口入口**：不直接写复杂业务逻辑，只接收参数、调用 Service、返回 `Result`。
3. **Service 层负责业务闭环**：例如创建决策时校验用户登录态、保存记录；提交回测时校验记录归属；生成 AI 建议时组合历史数据。
4. **Mapper 层保持轻量**：基于 MyBatis-Plus 完成单表操作，复杂统计查询单独写 XML 或 Wrapper。
5. **Common 层统一兜底**：统一维护 `BusinessException`，最终由 `GlobalExceptionHandler` 转成统一错误响应。包含 `Result`、`ErrorCode` 保证接口风格一致，便于前端统一处理。
